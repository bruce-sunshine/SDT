...
//	char name_list[100]= "hs-1d99-0001-1-2";
	printf("name_list = %s\n", name_list);
...
//		return 0;
//	}
	ENGINE_set_ex_data(e, 1, hd);
	printf("SDT_SKF engine init ok\n");
    return 1;
}
...
//  	printf("\nskf key len = %d\n", strlen(key));
	...
	{
		skf_rv = SKF_DecryptInit(sm4_ctx->hKeyHandle, sm4_ctx->Param);
		if(skf_rv != SAR_OK)
		{
			printf("skf, SKF_DecryptInit error, errorcode=[0x%08x]\n", skf_rv);
			return 0;
		}
	}
...
static const int hs_gmi_cipher_nids[] = {
    NID_sms4_ecb,
    NID_sms4_cbc,
    0
};
...
static int sdt_skf_sm4_cipher_nids_num = ((sizeof(hs_gmi_cipher_nids) - 1)  /
                                      sizeof(hs_gmi_cipher_nids[0]));

static int sdt_skf_sm4_ciphers(ENGINE *e, const EVP_CIPHER **cipher, const int **nids, int nid)
{
    /* No specific cipher => return a list of supported nids ... */
    if (!cipher) {
        *nids = hs_gmi_cipher_nids;
