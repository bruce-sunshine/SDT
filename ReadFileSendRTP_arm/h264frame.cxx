...
{
  _timestamp = 0;
  _maxPayloadSize = 1384;
  _encodedFrameConst = (uint8_t*)malloc(MAX_FRAME_SIZE);
  _encodedFrame = _encodedFrameConst;
  _NALs = NULL;
  _numberOfNALsReserved = 0;
  ////add by ck
  _fu_num=0;
  _getfirstpacketflag=0;
  _getlastpacketflag=0;
  ////add by ck
  BeginNewFrame();
}
...
void H264Frame::SetFromFrame (x264_nal_t *nal, int numberOfNALs) {
  int vopBufferLen;
  int currentNAL = 0;
  ...
  for (currentNAL = 0; currentNAL < numberOfNALs; currentNAL++) {
    int currentNALLen;
	currentNALLen=nal->i_payload;
    if (currentNALLen > 0) 
    {
      _NALs[_numberOfNALsInFrame].length = currentNALLen;
      _NALs[_numberOfNALsInFrame].offset = _encodedFrameLen;
      _NALs[_numberOfNALsInFrame].type =nal->i_type;
      uint32_t header = 0;
      if (currentPositionInFrame!=NULL&&IsStartCode(currentPositionInFrame))
      {
      ...
      _encodedFrame = nal->p_payload;
	  /*free(nal);*/
      if ( /*Trace::CanTraceUserPlane(4) &&*/ (nal->i_type == H264_NAL_TYPE_SEQ_PARAM)) 
      {
  ...
  { 
    ...
    else 
    {
      ...
      { 
        ...
        frame.SetMarker(((_currentNAL + 1) >= _numberOfNALsInFrame)&& !frame.GetNalPS ()? 1 : 0);
        flags |= frame.GetMarker() ? isLastFrame : 0;  // marker bit on last frame of video
		//ck revise for 8168 call
		_timestamp += frame.GetMarker() ? 3000 : 0;  // marker bit on last frame of video
...
///add by ck
bool H264Frame::Isgoodframe ()
{
	if(_fu_num>0)
	{
		if((_getfirstpacketflag==1)||(_getlastpacketflag==1))
			return true;
	}
	else
	{
